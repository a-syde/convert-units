...
tests['kPa to torr'] = function () {
  ...
  var actual = convert(532).from('kPa').to('torr')
...
tests['Pa to psi'] = function () {
  ...
  var actual = convert(10000).from('Pa').to('psi')
  assert.ok(percentError(expected, actual) < ACCURACY
    , 'Expected: ' + expected + ', Actual: ' + actual)
...
tests['torr to ksi'] = function () {
  var expected = 1
  var actual = convert(51714.931860168974).from('torr').to('ksi')
  assert.ok(percentError(expected, actual) < ACCURACY
    , 'Expected: ' + expected + ', Actual: ' + actual)
}
...
tests['psi to hPa'] = function () {
  ...
  var actual = convert(10).from('psi').to('hPa')
  assert.ok(percentError(expected, actual) < ACCURACY
    , 'Expected: ' + expected + ', Actual: ' + actual)
}
