...
tests['m to ft'] = function () {
  ...
  var actual = convert(1).from('m').to('ft')
...
tests['m to ft-us'] = function () {
  var expected = 3.28084
  var actual = convert(1).from('m').to('ft-us')
  assert.ok(percentError(expected, actual) < ACCURACY
    , 'Expected: ' + expected + ', Actual: ' + actual)
}
...
tests['mm to ft'] = function () {
  ...
  var actual = convert(1).from('mm').to('ft')
  assert.ok(percentError(expected, actual) < ACCURACY
    , 'Expected: ' + expected + ', Actual: ' + actual)
}
...
tests['nMi to m'] = function () {
  var expected = 1852
  var actual = convert(1).from('nMi').to('m')
  assert.ok(percentError(expected, actual) < ACCURACY
    , 'Expected: ' + expected + ', Actual: ' + actual)
}
...
tests['km to nMi'] = function () {
  ...
  var actual = convert(1).from('km').to('nMi')
  assert.ok(percentError(expected, actual) < ACCURACY
    , 'Expected: ' + expected + ', Actual: ' + actual)
}
...
tests['fathm to m'] = function () {
  var expected = 1.8288
  var actual = convert(1).from('fathom').to('m')
  assert.ok(percentError(expected, actual) < ACCURACY
    , 'Expected: ' + expected + ', Actual: ' + actual)
}
