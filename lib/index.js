...
let measures = {
  ...
  massVelocity: require('./definitions/massVelocity'),
  volumeVelocity: require('./definitions/volumeVelocity'),
  // TODO: write tests for definitions below
  volumeMassVelocity: require('./definitions/volumeMassVelocity'),
  massFlowRate: require('./definitions/massFlowRate'),
  volumeMass: require('./definitions/volumeMass'),
  massDistance: require('./definitions/massDistance'),
}
let Converter
  ...
  */
  if(this.origin.system !== this.destination.system) {
    ...
    if(typeof transform === 'function') {
      ...
      const ratio = measures[this.origin.measure]._anchors[this.origin.system].ratio 
      result *= ratio
    }
  }
...
Converter.prototype.toBest = function (options) {
  ...
  */
  each(this.possibilities(), function (possibility) {
    let unit = this.describe(possibility)
    let isIncluded = _options.exclude.indexOf(possibility) === -1

    if(isIncluded && unit.system === this.origin.system) {
      let result = this.to(possibility)
      if(!best || (result >= _options.cutOffNumber && result < best.val)) {
        best = {
          val: result,
          unit: possibility,
...
Converter.prototype.describe = function (abbr) {
  let resp = Converter.prototype.getUnit(abbr)
  let desc = null
